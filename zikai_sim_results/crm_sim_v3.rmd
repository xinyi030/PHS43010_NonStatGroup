...
This code uses Bayesian decision theory in conjunction with Bayesian logistic regressions. 
...
single_run <-  function(N, n_per_cohort, target, p_true, dose, plt = F,
                        loss = c(1, 0, 1, 2),tox.cutpoints = c(0.25, 0.36, 0.6)) {
  ncohort = N/n_per_cohort
...
selector <- as.numeric(est$ndose[[1]][1])
mtd <- dose[selector]
...
plt2 <- res2 %>% ggplot(mapping = aes(x = as.character(mtd)))+
  ...
  ylab("Proportion")+
    xlab("Dose")+
    ggtitle("Case1: Proportion of trials recommending each dose level")+
  theme_bw()  
plt2
...
plt4 <- res4 %>% ggplot(mapping = aes(x = as.character(mtd)))+
  ...
  ylab("Proportion")+
    xlab("Dose level")+
    ggtitle("Case2: Proportion of trials recommending each dose level")+
  theme_bw()  
plt4
